...
object PermissionUtils {
    ...
    private fun handlePermissionResult(
        ...
        callback: (granted: Boolean,isDoNotAsk: Boolean) -> Unit,
    ...
    fun requestActivityPermission(context: Context, callback: (granted: Boolean,isDoNotAsk: Boolean) -> Unit){
        val activity = getActivityFromContext(context)
        if (activity == null) {
            callback(false,false)
            return
        }
        val permission = PermissionLists.getActivityRecognitionPermission()
        XXPermissions.with(activity)
