...
object ChannelUserController {
    ...
    private const val KEY_USER_CHANNEL = "pdf_l8j4q2v9"
    private const val KEY_CHANNEL_SET_ONCE = "pdf_m1s7k6w3"
    ...
    private fun getDefaultChannelValue(): String {
        return try {
            ...
            if (UserChannelType.values().any { it.value == defaultChannel }) {
                CoreLogger.d("使用BuildConfig默认渠道: %s", defaultChannel)
                defaultChannel
            } else {
                CoreLogger.w("BuildConfig默认渠道无效: %s，使用NATURAL", defaultChannel)
                UserChannelType.NATURAL.value
            }
        } catch (e: Exception) {
            // 如果无法获取BuildConfig或字段不存在，使用默认值
            CoreLogger.e("获取BuildConfig默认渠道失败，使用NATURAL", e)
            UserChannelType.NATURAL.value
        }
    }
