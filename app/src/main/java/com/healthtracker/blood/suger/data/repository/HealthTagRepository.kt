    ...
    suspend fun initializePredefinedTags() {
        try {
            ...
            if (existingBloodSugarTags.isEmpty()) {
                // 插入血糖预定义标签
                val bloodSugarLabels = context.resources.getStringArray(com.healthtracker.blood.suger.R.array.ht_blood_sugar_labels)
            ...
            if (existingBloodPressureTags.isEmpty()) {
                // 插入血压预定义标签
                val bloodPressureLabels = context.resources.getStringArray(com.healthtracker.blood.suger.R.array.ht_blood_pressure_labels)
