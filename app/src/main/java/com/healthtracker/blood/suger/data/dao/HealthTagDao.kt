...
interface HealthTagDao {
     ...
     */
    @Query("SELECT * FROM health_tags WHERE id = :id")
     ...
     * @return 标签列表
     */
    @Query("SELECT * FROM health_tags WHERE id IN (:ids)")
     ...
     */
    @Query("SELECT * FROM health_tags WHERE tag_type = :tagType ORDER BY is_predefined DESC, create_time ASC")
     ...
     */
    @Query("SELECT * FROM health_tags WHERE tag_type = :tagType AND is_predefined = 1 ORDER BY predefined_index ASC")
     ...
     */
    @Query("SELECT * FROM health_tags WHERE tag_type = :tagType AND is_predefined = 0 ORDER BY create_time ASC")
     ...
     * @return 所有标签的Flow
     */
    @Query("SELECT * FROM health_tags ORDER BY tag_type ASC, is_predefined DESC, create_time ASC")
     ...
     * @return 是否存在
     */
    @Query("SELECT COUNT(*) > 0 FROM health_tags WHERE tag_type = :tagType AND name = :name")
     ...
     * @return 预定义标签数量
     */
    @Query("SELECT COUNT(*) FROM health_tags WHERE tag_type = :tagType AND is_predefined = 1")
     ...
     * @return 标签总数
     */
    @Query("SELECT COUNT(*) FROM health_tags WHERE tag_type = :tagType")
