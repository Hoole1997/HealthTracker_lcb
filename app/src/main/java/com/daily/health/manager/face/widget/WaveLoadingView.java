    ...
    private void updateWaveShader() {
        ...
        if (bitmapBuffer == null || haveBoundsChanged()) {
            ...
            if (width > 0 && height > 0) {
                ...
                wavePaint.setColor(mInnerColor);
...
//        mWaveShader = null;
    ...
    public void setProgressValue(int progress) {
        ...
        if (mProgressValue == safeProgress) {
            return;
        }
        mProgressValue = safeProgress;
    ...
    /**
     * 直接设置进度值，不带动画（用于页面初始化）
     *
     * @param progress 0-100 的进度值
     */
    public void setProgressValueNoAnimation(int progress) {
        int safeProgress = progress;
        if (safeProgress < 0) {
            safeProgress = 0;
        } else if (safeProgress > 100) {
