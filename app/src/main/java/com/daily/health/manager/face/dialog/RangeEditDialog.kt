...
import com.daily.health.manager.databinding.FcDialogRangeEditBinding
 ...
 * 血糖目标范围编辑对话框
 */
class RangeEditDialog : BaseBottomSheetDialogFragment<FcDialogRangeEditBinding>() {
    ...
    ) = FcDialogRangeEditBinding.inflate(inflater, parent, attachToParent)
    ...
    private fun saveRanges() {
        ...
        if (ranges == null) {
            showToast(getString(R.string.fc_please_input_complete_data))
            return
        }

        // 2. 验证范围顺序
        if (!validateRanges(ranges)) {
            showToast(getString(R.string.fc_range_order_error))
            return
        }

        // 3. 验证范围边界
        if (!validateBounds(ranges)) {
            showToast(getString(R.string.fc_range_bounds_error))
            return
        }
