...
class BloodPressureHistoryItem(private val record: BloodPressureRecord) : HistoryRecordItem() {
    ...
    override fun getUnit(): String {
        return App.INSTANCE.getString(R.string.fc_mmHg)
    }
    
    override fun getLevel(context: Context): String {
        val category = record.getBloodPressureCategoryEnum()
        return when (category) {
            BloodPressureCategory.LOW -> context.getString(R.string.fc_blood_pressure_level_low)
            BloodPressureCategory.NORMAL -> context.getString(R.string.fc_blood_pressure_level_normal)
            BloodPressureCategory.ELEVATED -> context.getString(R.string.fc_blood_pressure_level_elevated)
            BloodPressureCategory.HIGH_STAGE_1 -> context.getString(R.string.fc_blood_pressure_level_high_stage_1)
            BloodPressureCategory.HIGH_STAGE_2 -> context.getString(R.string.fc_blood_pressure_level_high_stage_2)
            BloodPressureCategory.HYPERTENSIVE_CRISIS -> context.getString(R.string.fc_blood_pressure_level_hypertensive_crisis)
