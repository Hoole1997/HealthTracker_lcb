...
class HydrateSettingAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            TYPE_DAILY -> DailyViewHolder(
                inflater.inflate(R.layout.fc_item_hydrate_setting_daily_intake, parent, false)
            )
            TYPE_CUP -> CupViewHolder(
                inflater.inflate(R.layout.fc_item_hydrate_setting_cup_size, parent, false)
            )
            TYPE_REMINDER -> ReminderContainerViewHolder(
                inflater.inflate(R.layout.fc_item_hydrate_setting_reminder_container, parent, false)
            )
            TYPE_ADD_REMINDER -> AddReminderViewHolder(
                inflater.inflate(R.layout.fc_item_hydrate_setting_add_reminder, parent, false)
            )
    ...
    class CupViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private fun updateSelectValueText(unitIsMl: Boolean, valueStr: String) {
            val unit = if (unitIsMl) R.string.fc_ml else R.string.fc_fl_oz
    ...
    inner class ReminderContainerViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind() {
            ...
            tvEdit.setText(if (isReminderEditMode) R.string.fc_cancel else R.string.fc_hydration_reminder_edit)
            ...
            btnAdd.setOnClickListener {
                ...
                AlarmTimeSelectDialog.show(activity.supportFragmentManager, null) { pair ->
                    ...
                    if (reminderTimes.contains(timeString)) {
                        Toast.makeText(activity, itemView.context.getString(R.string.fc_hydrate_reminder_exist), Toast.LENGTH_SHORT).show()
                        return@show
                    }
