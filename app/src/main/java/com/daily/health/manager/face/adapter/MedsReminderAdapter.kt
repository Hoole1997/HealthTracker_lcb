...
import com.daily.health.manager.databinding.FcItemMedsRemindBinding
import com.daily.health.manager.databinding.FcLayoutAdItemBinding
...
class MedsReminderAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == VIEW_TYPE_AD) {
            AdViewHolder(FcLayoutAdItemBinding.inflate(LayoutInflater.from(parent.context),parent,false))
        } else {
            val binding = FcItemMedsRemindBinding.inflate(
    ...
    inner class ReminderViewHolder(
        private val binding: FcItemMedsRemindBinding
        ...
        fun bind(item: MedsReminderItem) {
            with(binding) {
                ...
                if (!item.medicineCover.isEmpty()) {
                    Glide.with(ivStatu)
                        .applyDefaultRequestOptions(RequestOptions.placeholderOf(R.drawable.fc_ic_camera))
    ...
    inner class AdViewHolder(private val binding: FcLayoutAdItemBinding) : RecyclerView.ViewHolder(binding.root) {
        private lateinit var skeleton: ViewSkeletonScreen
        fun bind() {
            if (needLoadAd) {
                binding.root.visible()
                skeleton = ViewSkeletonScreen.Builder(binding.adContainer)
                    .load(R.layout.fc_layout_skeleton_banner_ads)
                    .shimmer(true)
                    .angle(30)
                    .duration(1200)
