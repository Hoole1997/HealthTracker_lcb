...
import com.daily.health.manager.databinding.FcActivityTargetRangeBinding
 ...
 * 血糖目标范围设置页面
 */
class TargetRangeAct : BaseMVVMActivity<TargetRangeViewModel, FcActivityTargetRangeBinding>() {

    private lateinit var adapter: TargetRangeAdapter
    private var hasChanged = false  // 标记是否有修改

    override fun createViewBinding() = FcActivityTargetRangeBinding.inflate(layoutInflater)
    ...
    private fun setupToolbar() {
        ...
        mViewBind.tvTitle.text = getString(R.string.fc_target_range_temp, BsUnit.getPreferredUnit().displayName)
    }
     ...
     * 显示编辑对话框
     */
    private fun showEditDialog(rangeItem: com.daily.health.manager.face.viewmodel.RangeItem) {
        RangeEditDialog.newInstance(
            ...
            onSave = { status, unit, ranges ->
                ...
                hasChanged = true
                showToast(getString(R.string.fc_save_success))
            }
    ...
    private fun showResetConfirmDialog() {
        ConfirmDialog(
            getString(R.string.fc_reset),
            getString(R.string.fc_reset_all_ranges_confirm),
            ...
            onDialogListener = object :DialogListener{
                override fun onItemClick(dialogFragment: DialogFragment, which: Int) {
                    ...
                    if(which == R.id.btn_ok){
                        ...
                        hasChanged = true
                        showToast(getString(R.string.fc_reset_success))
                    }
                }
            }
