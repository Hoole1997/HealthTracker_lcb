...
class ChoosePhotoRCVAdapter(private val data: List<String>, private val listener: ChoosePhotoRCVListener) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerViewHolder {
        ...
        return RecyclerViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.fc_feedback_item_rcv_photo, parent, false))
    }
    ...
    override fun onBindViewHolder(holder: RecyclerViewHolder, position: Int) {
        when (position) {
            data.size -> {
                //拍照或选择图片 触发按钮
                holder.photoIV.setImageResource(R.drawable.fc_feedback_add_photo_dark)
