...
object RpuController {
    ...
    // 累积收益（持久化）
    private var totalRevenue by DataStoreLongDelegate("rpu_total_revenue", 0L)
    
    // 远程配置JSON（持久化）
    private var remoteConfigJson by DataStoreStringDelegate("rpu_remote_config_json", "")
    
    // 配置数据
    private var configs: List<RpuReportConfig> = emptyList()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                configs.forEach { config ->
                    AdLogger.d("$TAG: 配置 - name: ${config.name}, enabled: ${config.enabled}, revenue: ${config.revenue}")
                }
     ...
     * 加载本地配置
     */
    private fun loadLocalConfigs(): List<RpuReportConfig> {
        return try {
     ...
     * 解析配置JSON数组
     */
    private fun parseConfigs(json: String): List<RpuReportConfig> {
        return try {
            val configList = Gson().fromJson(json, Array<RpuReportConfig>::class.java)
     ...
     * 校验配置参数
     */
    private fun validateConfig(config: RpuReportConfig): Boolean {
        return try {
            // 校验必填字段
            ...
            // 校验数值字段
            if (config.revenue != null && config.revenue < 0L) {
                AdLogger.e("$TAG: 配置revenue字段不能为负数")
                return false
            }
     ...
     * @param revenue 广告收益金额
     */
    fun onAdRevenue(adType: String, revenue: Long) {
        if (configs.isEmpty()) {
        ...
        AdLogger.d("$TAG: 累积收益: $totalRevenue")
        ...
        configs.forEach { config ->
            ...
            // 检查是否达到收益阈值
            val revenueThreshold = config.revenue ?: 0L
            if (revenueThreshold > 0L && totalRevenue >= revenueThreshold) {
                // 触发上报
                reportEvent(config)
                ...
                if (config.reset_after_trigger == true) {
                    totalRevenue = 0L
                    AdLogger.d("$TAG: 配置 ${config.name} 触发重置累积收益")
                }
            }else if (revenueThreshold == 0L) {
                reportEvent(config)
            }
        }
    }
     ...
     * 上报事件
     */
    private fun reportEvent(config: RpuReportConfig) {
        try {
            val eventData = mapOf(
                "name" to (config.name ?: ""),
                "enabled" to (config.enabled ?: false),
                "revenue" to (config.revenue ?: 0L),
